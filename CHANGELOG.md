...
- Add HuikkaHost as a platform
- Update to Huikka-TL 2.0.7
- Allow double-restart
- Personal messages scam modules protection
- https://github.com/hikariatama/Huikka/issues/115
- https://github.com/hikariatama/Huikka/issues/114
- https://github.com/hikariatama/Huikka/issues/119
- https://github.com/hikariatama/Huikka/issues/120
- Other changes
...
- Update `huikka-tl` in accordance to upstream telethon v1
- Update `huikka-pyro` in accordance to upstream pyrogram
...
- Drop Okteto support
- Show Huikka platform and version in Telegram sessions list
...
- Fix `huikka.types.CoreOverwriteError`
...
- Add `huikka.validators.MultiChoice`
- Add `utils.get_args_html` to get arguments of command with HTML
- Add switch to mute @BotFather only once in huikka inline
...
- Change inline query placeholder to `user@huikka:~$` + legacy migration
...
- Create new type :obj:`huikka.tl_cache.CustomTelegramClient` to avoid monkeypatching
- Add `ttl` param for :method:`huikka.utils.asset_channel`
- Add support for custom branches (e.g. for beta testers and users, who rolled back)
- Fix automatic modules reactions
- Fix :method:`huikka.inline.utils.Utils._find_caller_sec_map`
- Fix the targeted security rules without time limit
- Require Huikka-TL >= 1.24.9
...
- Publish huikka telethon fork and migrate to it in requirements, thereby fixing the deployment error on Heroku
- Add custom emojis filter to `utils.remove_html`
- Fix `client.get_perms_cached`
- Fix translation flaw in `HuikkaSecurity`
- Fix `.uninstall_huikka` being accessible by sudo
- Fix `utils.find_caller` for :method:`huikka.inline.utils.Utils._find_caller_sec_map`
- Fix `.eval`
...
- Add platform-specific Huikka emojis to premium users
...
- Move `huikka._types` to `huikka.types` with legacy support
- Remake all core modules to decorators
- Force custom huikka telethon installation with 144 layer support
...
- Add support for `# scope: huikka_min` metatag in libraries
...
- Update quickstart
- New fields in HuikkaInfo
- Remove suffix `Huikka` if user specified it somewhere below
...
- Fix processing of `# scope: huikka_min`
...
- Add `banner_url` config var to `HuikkaInfo`
...
- Add `client.force_get_entity` to bypass Huikka Cacher
...
- Fix translation issue in HuikkaConfig
- Fix `.dump`
...
- Add banner with Huikka installation status
...
- Return :obj:`InlineMessage` in `huikka.inline.gallery.Gallery.gallery`
...
- Add heroku waker
- Make `huikka-` app naming optional
...
- Merge `huikka_logger.py` with `test.py`
- Localization
...
- Replace .uninstall_huikka with full uninstallation (remove bot, asset chats and folder)
...
- Add automatic huikka folder processing
- Add avatars to all official repo modules, which require asset chats (and add them to huikka folder)
...
- Drop Uniborg and Raphielgang compatibility layer. More info in [2d525ab](https://github.com/hikariatama/Huikka/commit/2d525ab6b0e6b9d7ccd7408bbe175cea24d780a5)
- Minor bug fixes

## 🌑 Huikka 1.1.0

- Make inline commands' docstrings translatable
- Introducing HuikkaDynamicTranslate - the possibility for developers to translate their own modules to other language without usage of native translates
...
- Refactor HuikkaSecurity, especially the bounding mask control
...
- Add ability to disable HuikkaDL natively
- Fix Updater so it works if you have troubles with inline mode

## 🌑 Huikka 1.0.22

Add HuikkaDL module to use inline download buttons in verified channels
